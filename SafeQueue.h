...
template<class T, class _Container = std::vector<T>, class _Pr = std::less<typename _Container::value_type>>
class SafePriorityQueue
{

private:
    //std::queue<T> q;
    std::priority_queue<T, _Container, _Pr> q;
    std::mutex m;
    std::condition_variable c;
    bool mIsLoop = true;
    bool bEscape;
...
public:
    SafePriorityQueue()
        ...
        , bEscape(false)
    {}
    ~SafePriorityQueue(void) {};

    SafePriorityQueue(SafePriorityQueue<T, _Container, _Pr>&& other)
        :q(std::move(other.q))
    {
    }
